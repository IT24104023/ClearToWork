  ...
  return (
    <div className="p-6 space-y-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-center space-x-3 border-b border-slate-800 pb-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-white tracking-tight">{t('profile_title')}</h1>
          <p className="text-xs text-slate-400 mt-0.5">{t('profile_subtitle')}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
        ...
        <div className="md:col-span-4 space-y-4">
          <div className="bg-slate-900 dark:bg-slate-900/80 p-5 rounded-2xl border border-slate-800 text-center shadow-md">
            <div className="relative inline-block mb-3">
              ...
              />
              <div className="absolute bottom-0 right-0 bg-slate-950 text-amber-400 p-1.5 rounded-full border border-amber-400/60 shadow">
                <Camera className="w-3.5 h-3.5" />
              </div>
            </div>

            <h2 className="text-base font-bold text-white leading-tight">{fullName || 'User'}</h2>
            <div className="text-xs font-mono text-amber-400 mt-1 uppercase tracking-wider">{role}</div>
            <div className="text-xs text-slate-400 mt-1">{email}</div>

            {/* Preset Avatars Picker */}
            <div className="mt-4 pt-4 border-t border-slate-800">
              <div className="text-[11px] font-semibold text-slate-400 mb-2">{t('profile_choose_avatar')}</div>
                ...
                {PRESET_AVATARS.map((url, i) => (
                  <img
                    ...
                    className={`w-7 h-7 rounded-full object-cover cursor-pointer border transition-transform hover:scale-110 ${
                      avatarUrl === url ? 'border-amber-400 ring-2 ring-amber-400/40' : 'border-slate-700 opacity-60'
                    }`}
                  />
                ))}
              </div>
        ...
        <div className="md:col-span-8 space-y-6">
          <form onSubmit={handleSave} className="bg-slate-900 dark:bg-slate-900/80 p-6 rounded-2xl border border-slate-800 shadow-md space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full bg-slate-950 border border-slate-700 rounded-xl px-3 py-2 text-xs text-white focus:outline-none focus:border-amber-400 leading-relaxed"
              />
            </div>
          ...
          {/* Permissions Matrix */}
          <div className="bg-slate-900 dark:bg-slate-900/80 p-6 rounded-2xl border border-slate-800 shadow-md">
            <div className="flex items-center space-x-2 mb-2">
              <KeyRound className="w-4 h-4 text-amber-400" />
              <h3 className="text-sm font-bold text-white">{t('profile_permissions_title')}</h3>
            </div>
            <p className="text-[11px] text-slate-400 mb-4">{t('profile_permissions_desc')}</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
              {permissions.map((perm) => (
                ...
                >
                  <span className="font-mono text-xs text-emerald-400 font-semibold">{perm}</span>
