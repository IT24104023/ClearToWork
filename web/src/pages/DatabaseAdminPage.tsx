    ...
    <div className="p-4 sm:p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-4">
        <div className="flex items-center space-x-3">
          <div className="bg-amber-500/20 text-amber-500 dark:text-amber-400 p-2.5 rounded-2xl border border-amber-500/30 shadow-sm">
            <Database className="w-6 h-6" />
          </div>
          <div>
            <h1 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">{t('db_admin_title')}</h1>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{t('db_admin_subtitle')}</p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 sm:gap-3">
          <button
            ...
            disabled={loading}
            className="flex items-center space-x-1.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 px-3 py-2 rounded-xl text-xs font-semibold border border-slate-200 dark:border-slate-700 transition-colors"
          >
          ...
          <button
            ...
            disabled={loading}
            className="flex items-center space-x-1.5 bg-rose-50 dark:bg-rose-950/40 hover:bg-rose-100 dark:hover:bg-rose-900/60 text-rose-700 dark:text-rose-300 border border-rose-300 dark:border-rose-800/60 px-3.5 py-2 rounded-xl text-xs font-bold transition-colors"
          >
      ...
      {summary && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
            ...
            <div className="text-lg font-bold text-slate-900 dark:text-white">{summary.provider}</div>
            <div className="text-[11px] text-slate-400 dark:text-slate-500 mt-1">ORM: Entity Framework Core 8</div>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
            ...
            <div className="text-lg font-bold text-amber-600 dark:text-amber-400 font-mono">{summary.sizeFormatted}</div>
            <div className="text-[11px] text-slate-400 dark:text-slate-500 mt-1">File: {summary.databaseFile}</div>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
            ...
            <div className="text-lg font-bold text-emerald-600 dark:text-emerald-400">{summary.status}</div>
            <div className="text-[11px] text-slate-400 dark:text-slate-500 mt-1">WAL Journaling Active</div>
          </div>
        </div>
      )}
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-sm">
          <div className="px-5 py-4 bg-slate-50 dark:bg-slate-950/60 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
            <div className="flex items-center space-x-2">
              <Table className="w-4 h-4 text-amber-500 dark:text-amber-400" />
              <h2 className="font-bold text-slate-900 dark:text-white text-sm">Entity Relational Record Counts</h2>
            </div>
          ...
          <div className="p-5 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
            {Object.entries(summary.tableCounts).map(([table, count]) => (
              <div key={table} className="p-3 bg-slate-50 dark:bg-slate-950/70 border border-slate-200 dark:border-slate-800/80 rounded-xl">
                <div className="text-[11px] text-slate-600 dark:text-slate-400 truncate mb-1" title={table}>{table}</div>
                <div className="text-xl font-bold font-mono text-slate-900 dark:text-white">{count}</div>
                <div className="text-[10px] text-emerald-600 dark:text-emerald-400 mt-0.5">Rows Verified</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-3xl p-6 max-w-md w-full space-y-4 shadow-2xl">
            <div className="flex items-center gap-3 text-rose-500">
              ...
              <h3 className="font-bold text-slate-900 dark:text-white text-lg">Confirm Database Recreate</h3>
            </div>
