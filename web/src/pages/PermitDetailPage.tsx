  ...
  if (error || !permit) {
    return (
      <div className="p-8 text-center text-rose-400 space-y-3">
        ...
        >
          <ArrowLeft className="w-3 h-3" /> Return to Permit Roster
        </button>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto pb-12">
      ...
      <div className="flex items-center justify-between">
        ...
        >
          <ArrowLeft className="w-4 h-4" /> Back to Permit Register
        </button>
        ...
        <div className="flex items-center gap-3">
          {permit.status === 'Draft' && (
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-purple-600 hover:bg-purple-500 text-white font-bold rounded-lg text-xs shadow-md transition disabled:opacity-50"
            >
              <Cpu className="w-4 h-4" />
              <span>{isReviewing ? 'Running Agents...' : 'Run Agentic AI Clearance'}</span>
            </button>
          )}
          ...
          {canDecide && (
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-lg text-xs shadow-md transition"
            >
              <ShieldCheck className="w-4 h-4" />
              <span>Sign-Off Safety Clearance</span>
            </button>
          )}
        </div>
      </div>
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          {permit.permitQrToken && (
            <div className="bg-slate-950 p-2.5 rounded-lg border border-slate-800 flex items-center gap-2 text-xs">
              ...
              <div>
                <div className="text-[10px] text-slate-400 font-medium uppercase">Active Permit Token</div>
                <div className="font-mono text-emerald-300 font-bold text-[11px]">{permit.permitQrToken}</div>
              </div>
            </div>
          )}
        </div>

        <p className="mt-4 text-sm text-slate-300 bg-slate-950/60 p-3 rounded-lg border border-slate-800 leading-relaxed">
          {permit.objectiveDescription}
        </p>

        {/* Metadata Badges */}
        <div className="mt-4 grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs pt-4 border-t border-slate-800/80">
          <div className="flex items-center gap-2 text-slate-300">
            <MapPin className="w-4 h-4 text-amber-400 shrink-0" />
            <div>
              <span className="text-slate-500 block text-[10px] uppercase">Zone Location</span>
          ...
          <div className="flex items-center gap-2 text-slate-300">
            ...
            <div>
              <span className="text-slate-500 block text-[10px] uppercase">Time Window</span>
          ...
          <div className="flex items-center gap-2 text-slate-300">
            ...
            <div>
              <span className="text-slate-500 block text-[10px] uppercase">Field Supervisor</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-6 lg:col-span-1">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 shadow-sm">
            <div className="flex items-center gap-2 text-sm font-bold text-slate-200 pb-3 border-b border-slate-800">
              <HardHat className="w-4 h-4 text-amber-400" />
              <span>Assigned Personnel ({permit.assignedWorkers.length})</span>
            </div>

            <div className="mt-3 space-y-2.5">
              {permit.assignedWorkers.length === 0 ? (
                <div className="text-xs text-slate-500 italic py-2">No workers assigned yet.</div>
              ) : (
                permit.assignedWorkers.map((w) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="text-[11px] text-slate-400 mt-0.5">{w.trade} · {w.contractorName}</div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 shadow-sm">
            <div className="flex items-center gap-2 text-sm font-bold text-slate-200 pb-3 border-b border-slate-800">
              ...
              <span>Safety Equipment ({permit.assignedAssets.length})</span>
            </div>

            <div className="mt-3 space-y-2.5">
              {permit.assignedAssets.length === 0 ? (
                <div className="text-xs text-slate-500 italic py-2">No equipment assigned.</div>
              ) : (
                permit.assignedAssets.map((asset) => (
                  ...
                  >
                    ...
                    <div className="text-[11px] text-slate-400 mt-0.5">{asset.category}</div>

                    <div className="mt-1.5 grid grid-cols-2 gap-1 text-[10px] font-mono">
                      <div>
                        Insp:{' '}
                        <span className={asset.isInspectionValid ? 'text-emerald-400 font-bold' : 'text-rose-400 font-bold'}>
                          {asset.isInspectionValid ? 'VALID' : 'OVERDUE'}
                        </span>
                      </div>
                      <div>
                        Calib:{' '}
                        <span className={asset.isCalibrationValid ? 'text-emerald-400 font-bold' : 'text-rose-400 font-bold'}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-lg w-full p-6 shadow-2xl space-y-4">
            ...
            <p className="text-xs text-slate-400">
              You are certifying safety clearance for Permit <strong className="text-white">{permit.permitNumber}</strong> on {permit.zoneName}.
            </p>
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full p-2.5 bg-slate-950 border border-slate-700 rounded-lg text-xs text-white placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-amber-500"
              />
            </div>
            ...
            <div className="flex items-center justify-end gap-3 pt-3 border-t border-slate-800">
              ...
              <button
                ...
                className="px-4 py-2 bg-amber-500 hover:bg-amber-400 text-slate-950 text-xs font-bold rounded-lg transition disabled:opacity-50"
              >
